...
#include <assert.h>
#include <ctype.h>
#ifdef LOGTOFILE_JXQY
#include <fstream>
#endif // LOGTOFILE_JXQY
...
static void ColouriseJxqyDoc(unsigned int startPos, int length, int initStyle, WordList *keywordlists[], Accessor &styler)
{
#ifdef LOGTOFILE_JXQY
    std::ofstream of("e:\\temp\\debug.txt");
    of << "startPos" << startPos << " length:" << length<< " inistyle:" << initStyle;
#endif // LOGTOFILE_JXQY

    WordList &keywordlag = *keywordlists[0];
    WordList &keywordfun = *keywordlists[1];
    CharacterSet alphaSet(CharacterSet::setAlpha);
    ...
    {
#ifdef LOGTOFILE_JXQY
        of << "\nstate:" << sc.state << " atLineEnd:" << sc.atLineEnd << " atLineStart:" << sc.atLineStart
            << " sc.ch:" << sc.ch << " sc.chNext:" << sc.chNext << " sc.currentPos:" << sc.currentPos;
#endif // LOGTOFILE_JXQY
        atEnd = (sc.currentPos == (startPos + length - 1));

        switch(sc.state)
        {
        case SCE_JXQY_COMMENT:
            if(sc.atLineEnd)
            {
                #ifdef LOGTOFILE_JXQY
                of << "\nmatch newline";
                #endif // LOGTOFILE_JXQY
                sc.SetState(SCE_JXQY_DEFAULT);
                break;
            }
            break;
        case SCE_JXQY_IDENTIFIER:
            while(alphaSet.Contains(sc.ch))
            {
                sc.Forward();
            }
            {
                ...
                if(keywordlag.InList(buf))
                    sc.ChangeState(SCE_JXQY_KEYWORD);
                if(keywordfun.InList(buf) &&
                   (sc.ch == ';' || sc.ch == '(' || IsASpace(sc.ch))
                    )
                    sc.ChangeState(SCE_JXQY_FUNCTION);
            }
            sc.SetState(SCE_JXQY_DEFAULT);

            break;
        case SCE_JXQY_STRING:
            if(sc.Match('\"'))
                sc.ForwardSetState(SCE_JXQY_DEFAULT);
            if(sc.atLineEnd)
                sc.SetState(SCE_JXQY_DEFAULT);
            break;
        case SCE_JXQY_GOTOPOS:
            if(!IsAlphaNumeric(sc.ch))
                sc.SetState(SCE_JXQY_DEFAULT);
            break;
        case SCE_JXQY_VARIABLE:
            if(!IsAlphaNumeric(sc.ch))
                sc.SetState(SCE_JXQY_DEFAULT);
            break;
        case SCE_JXQY_NUMBER:
            if(!IsADigit(sc.ch) && sc.ch != '.')
                sc.SetState(SCE_JXQY_DEFAULT);
            break;
        case SCE_JXQY_OPERATOR:
            sc.SetState(SCE_JXQY_DEFAULT);
            break;
        }

        if(sc.state == SCE_JXQY_DEFAULT)
        {
            char buf[1000];
            sc.GetCurrent(buf, sizeof(buf));
            #ifdef LOGTOFILE_JXQY
            ...
            if(sc.Match('/', '/'))
            {
                ...
                of << "\nmatch \\\\";
                #endif // LOGTOFILE_JXQY
                sc.SetState(SCE_JXQY_COMMENT);
                sc.Forward();
            }
            else if(alphaSet.Contains(sc.ch))
            {
                sc.SetState(SCE_JXQY_IDENTIFIER);
            }
            else if(sc.Match('\"'))
            {
                sc.SetState(SCE_JXQY_STRING);
            }
            else if(sc.Match('@'))
            {
                sc.SetState(SCE_JXQY_GOTOPOS);
            }
            else if(sc.Match('$'))
            {
                sc.SetState(SCE_JXQY_VARIABLE);
            }
            else if(IsADigit(sc.ch))
            {
                sc.SetState(SCE_JXQY_NUMBER);
            }
            else if(isoperator(sc.ch))
            {
                sc.SetState(SCE_JXQY_OPERATOR);
            }
        }
    }
    #ifdef LOGTOFILE_JXQY
...
static const char *const jxqyWordListDesc[] = {
    "Language keywords",
    "Function keywords",
    0
};
...
LexerModule lmJxqy(SCLEX_JXQY, ColouriseJxqyDoc, "jxqy", 0, jxqyWordListDesc);
