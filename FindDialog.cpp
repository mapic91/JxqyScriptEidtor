...
FindDialog::FindDialog(wxWindow* parent,wxWindowID id,const wxPoint& pos,const wxSize& size)
{
    ...
    m_button_Replace_Next->SetDefault();
    m_button_Replace_Replace = new wxButton(m_panel_Replace, ID_BUTTON4, _T("替换(&R)"), wxPoint(264,80), wxSize(82,23), 0, wxDefaultValidator, _T("ID_BUTTON4"));
    m_button_Replace_All = new wxButton(m_panel_Replace, ID_BUTTON5, _T("替换所有(&A)"), wxPoint(264,112), wxSize(82,23), 0, wxDefaultValidator, _T("ID_BUTTON5"));
...
bool FindDialog::DoReplace(const wxString& findVal, bool toPos, int *counts)
{
    ...
    if(m_stc)
    {
        ...
        if(findText.Cmp(selectText) == 0)
        {
            ...
            m_lineFindBegin = CharPosToStrPos(m_stc->GetLine(m_lineNumber), selStart - lineStartPos);
            if(isDown)
            {
                m_lineFindBegin += replaceText.Length();
            }
            else
            {
                if(m_lineFindBegin != 0)
					m_lineFindBegin--;
				else
				{
					m_lineNumber--;
...
void FindDialog::Onm_button_Replace_AllClick(wxCommandEvent& event)
{
    ...
    if(m_stc)
    {
        ...
        int curViewLine = m_stc->LineFromPosition(m_stc->PositionFromPoint(wxPoint(0, 0)));

        bool wrapAround = m_checkBox_Wraparound->GetValue();
        if(wrapAround)
		{
			//temporary disable wraparound
			m_checkBox_Wraparound->SetValue(false);
        ...
        while(DoReplace(findText, false, &counts))
		m_stc->EndUndoAction();

		m_stc->ScrollToLine(curViewLine);
		if(wrapAround) m_checkBox_Wraparound->SetValue(true);
    ...
    if(event.GetActive())
    {
        if(m_edit)
        {
            ...
            if(m_stc)
            {
                ...
                m_lineFindBegin = CharPosToStrPos(m_stc->GetLine(m_lineNumber), m_lineFindBegin);
...
bool FindDialog::BeginFind(const wxString& findVal, bool toPos)
{
    ...
    bool isNoCase = !(m_checkBox_MatchCase->GetValue());
    bool finded = false;
    size_t pos, pos_char, findTextLength_char;
    int lineCount = m_stc->GetLineCount();
    wxString line;
    wxString findText = findVal;
    if(isNoCase) findText.MakeLower();
    findTextLength_char = findText.utf8_str().length();
    for(;
            ...
            isDown ? m_lineNumber++ : m_lineNumber--)
    {
        ...
        {
        	pos_char = line.Mid(0, pos).utf8_str().length();
            finded = true;
            int lineBegPos = m_stc->PositionFromLine(m_lineNumber);
            //if(toPos)m_stc->GotoPos(lineBegPos + pos);
            m_stc->SetSelection(lineBegPos + pos_char, lineBegPos + pos_char + findTextLength_char);
